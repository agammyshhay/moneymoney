...
export default function Account({ account, actionButtons }: AccountProps) {
  ...
  return (
    <div className={containerStyles.join(' ')}>
      ...
      {actionButtons?.map(({ icon, clickHandler, tooltipText, alwaysVisible }) => (
        <i
          className={`bi ${icon} ${styles.actionButton} ${alwaysVisible ? styles.actionButtonVisible : ''}`}
      ...
      {badgeNumberLog?.originalEvent && (
        <Badge className={styles.newTxnsIndicator} bg={'success'} title="תנועות שיוצאו">
